...
//application.log.finest( "index of RM ##D : " + fileRec.recordText.startsWith("RM ##D"));
					if (fileRec.recordID.equals("T-")) {
						...
						// it is conjunction/linked ticket
						...
						} // If no conjunction ticket it will be defaulted to
						  // ticket number
					...
					// amount else pct,FM*C*1;S3-4,6-7;P1-2 OR FM*M*8
						...
						int endPos = fileRec.recordText.indexOf(";"); // returns -1
																   // if not
																   // found
						if (endPos < 0) {
							endPos = fileRec.recordText.length();
						} // Here if no semicolon means it is manual commission
						  // eg FM*M*8

                        if (fileRec.recordText.substring(endPos - 1, endPos)
                                .equalsIgnoreCase("P")) { // "P" commission pct
                            recCommPct = fileRec.recordText.substring(
                                    startPos + 1, endPos - 1);
                        } else if (fileRec.recordText.substring(endPos - 1, endPos)
                        ...
                        if (fileRec.recordID.equals("FP") && (StringUtils.searchStringOccur(fileRec.recordText, "NONREFAGT", 1) > 0)) {
                            ...
                            if ((refPos > 0) && (fileRec.recordText.length() > refPos + 9)) {
                                if ((fileRec.recordText.charAt(refPos + 9) >= 'A') &&
                                    ...
                                    int delimPos = fileRec.recordText.indexOf("/"); // returns -1, if not found

                                    if (delimPos == 3) {
                                        delimPos = StringUtils.searchStringOccur(fileRec.recordText, "/", 2); // returns -1, if not found
                                    }
				...
				for (int i = 1; i <= noOfConjTkts; i++) {
					...
					if (i > 1) {
						...
						if (exchangeFound) {
							// Get the next Exchange ticket number it this is a
							// exchange of conjuction ticket
