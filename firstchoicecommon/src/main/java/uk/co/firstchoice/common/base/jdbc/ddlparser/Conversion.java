...
public class Conversion {
    ...
    private static int[] ebcasc = { 0xD8, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66,
            0x67, /* 80 - 87 */
    ...
    private static int[] ascebc = { 0x20, 0x21, 0x22, 0x23, 0x24, 0x15, 0x06,
            0x17, /* 80 - 87 */
    ...
    static {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (int j = k; j < (k + 16); j++) {
                ...
                //
                // Call getHex with a different number upto 16 every time loop
                // is executed
                //
                hex_bot[j] = "";
                bot = getHex(l);
    ...
    public static int byteToInteger(byte[] data, int data_ptr) {
        ...
        return (st_res1 * 16777216) + (st_res2 * 65536) + (st_res3 * 256)
                + st_res4;
    }
    ...
    public static String[] stringToHex(String s, int offset) {
        ...
        //
        // The String may have a prefix which the caller may not wish to be
        // converted
        ...
        for (i = 0; i < b.length; i++) {
            ...
            int bi = by.intValue();
            //
            // Convert ints to hex taking care of any signed values at the same
            // time
            //
        ...
        //
        hex[0] = String.valueOf(c_top);
        hex[1] = String.valueOf(c_bot);
    ...
    //
    // Methods to check if an argument is a valid hex format, then compare it
    // with
    // another hex value
    //
